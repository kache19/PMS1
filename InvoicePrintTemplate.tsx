...
const InvoicePrintTemplate: React.FC<InvoicePrintTemplateProps> = ({ invoice, companySettings }) => (
  <PrintLayout title={invoice.status === 'PAID' ? 'TAX INVOICE' : 'PROFORMA INVOICE'}>
    <div style={{ textAlign: 'center', marginBottom: 24 }}>
      ...
      <div style={{ fontSize: 12, color: '#555' }}>TIN: {companySettings.tinNumber} {companySettings.vrnNumber && `| VRN: ${companySettings.vrnNumber}`}</div>
    ...
    {invoice.status === 'PAID' && (
      <div style={{ margin: '24px 0', textAlign: 'center', border: '2px solid #222', padding: 12, fontWeight: 'bold', fontSize: 18 }}>PAID IN FULL</div>
    )}
