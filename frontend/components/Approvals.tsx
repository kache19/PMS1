...
const Approvals: React.FC<ApprovalsProps> = ({
  ...
  const handleTransferApproval = async (transfer: StockTransfer) => {
    try {
      // Call API to approve transfer
      await api.approveTransfer(transfer.id);

      // Update local state - change status to COMPLETED and remove from pending list
            ...
            {pendingTransfers.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {pendingTransfers.map(transfer => {
                  ...
                  const canApprove = ['STOREKEEPER', 'INVENTORY_CONTROLLER', 'BRANCH_MANAGER', 'SUPER_ADMIN'].includes(currentUserRole || '');
                        ...
                        <div className="flex gap-2">
                          {canApprove && (
                            <button
                              ...
                              type="button"
                            >
                              <CheckCircle size={16} /> Complete Transfer
                            </button>
                          )}
                        </div>
                      </div>
