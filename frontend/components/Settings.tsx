...
const Settings: React.FC<SettingsProps> = ({
    ...
    const loadData = async () => {
      try {
        ...
        if (settingsData && settingsData.length > 0) {
          const logoValue = settingsData.find((s: any) => s.settingKey === 'logo')?.settingValue;
          const logoUrl = logoValue ? `https://malenyapharmacy.com${logoValue}` : null;
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Save the logo path immediately
      try {
        const existingSetting = settings.find(s => (s as any).settingKey === 'logo' && s.category === 'general');
        if (existingSetting) {
          await api.updateSetting(String(existingSetting.id), result.relativePath);
        } else {
          await api.createSetting({
            id: 'logo',
            category: 'general',
            settingKey: 'logo',
            settingValue: result.relativePath,
            dataType: 'string',
            description: 'Company logo setting'
          });
        }
        // Refresh settings to ensure consistency
        const updatedSettings = await api.getSettings();
        setSettings(updatedSettings || []);
        showSuccess('Logo Uploaded', 'Company logo has been uploaded and saved successfully.');
      } catch (error) {
        console.error('Failed to save logo setting:', error);
        showError('Save Failed', 'Logo uploaded but failed to save settings. Please try saving manually.');
      }
    } catch (error) {
      console.error('Logo upload error:', error);
      showError('Upload Failed', error instanceof Error ? error.message : 'Failed to upload logo.');
    }
