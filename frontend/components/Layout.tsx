...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const rolePermissions: Record<UserRole, string[]> = {
    ...
    [UserRole.DISPENSER]: ['dashboard', 'pos', 'clinical'],
    [UserRole.STOREKEEPER]: ['dashboard', 'inventory'],
  ...
  return (
    <div id="app-layout" className="flex flex-col h-screen bg-slate-50 overflow-hidden">
      ...
      <header className="hidden md:flex items-center justify-between px-6 py-3 bg-white border-b border-slate-200 shadow-sm no-print">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Store size={20} className="text-teal-600" />
            <div>
              <p className="text-sm font-semibold text-slate-800">{activeBranchName}</p>
              <p className="text-xs text-slate-500">{activeBranchLocation}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={onLogout} className="p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors" title="Logout">
            <LogOut size={18} />
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col overflow-hidden md:flex-1">
        <header className="md:hidden flex items-center justify-between p-4 bg-teal-900 text-white shadow-md z-20 no-print">
          <div>
            ...
            <p className="text-xs text-teal-300">{activeBranchName} - {activeBranchLocation}</p>
          </div>
          ...
          <div className="max-w-7xl mx-auto p-4 md:p-8">
             ...
             <div className="mb-6 flex items-center justify-between bg-white p-3 rounded-lg shadow-sm border border-slate-200 no-print">
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-bold ${isHeadOffice ? 'bg-teal-100 text-teal-800' : 'bg-blue-100 text-blue-800'}`}>
