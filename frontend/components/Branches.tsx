...
const Branches: React.FC<BranchesProps> = ({ branches, onUpdateBranches, onAddBranch, staff = [], currentUser }) => {
  ...
  const handleSetAsHeadOffice = async (branch: Branch) => {
    try {
      await api.request(`/branches/${branch.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ is_head_office: true })
      });

      // Update local state
      const updatedBranches = branches.map(b => ({
        ...b,
        isHeadOffice: b.id === branch.id ? true : false
      }));
      onUpdateBranches(updatedBranches);

      showSuccess('Head Office Updated', `${branch.name} has been set as the head office.`);
    } catch (error: any) {
      console.error('Failed to set head office:', error);
      showError('Update Failed', error.message || 'There was an error setting the head office. Please try again.');
    }
  };
                ...
                <tr key={branch.id} className="hover:bg-slate-50">
                   ...
                   <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {currentUser?.role === 'SUPER_ADMIN' && branch.id !== (mainBranch?.id || 'HEAD_OFFICE') && !branch.isHeadOffice && (
                          <button
                            onClick={() => handleSetAsHeadOffice(branch)}
                            className="p-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:text-amber-600 hover:border-amber-200 hover:bg-amber-50 transition-colors"
                            title="Set as Head Office"
                          >
                            <Crown size={16} />
                          </button>
                        )}
                        {currentUser?.role === 'SUPER_ADMIN' && branch.id !== (mainBranch?.id || 'HEAD_OFFICE') && (
                          <button
