...
function createPatient() {
    ...
    try {
        error_log('DEBUG: Starting createPatient function');
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'PHARMACIST', 'CASHIER', 'DISPENSER']);
        ...
        error_log("DEBUG: Parsed data - id: '$id', name: '$name', branchId: '$branchId'");

        $user = getCurrentUser();
        $createdBy = $user['id'] ?? 'SYSTEM';
        error_log("DEBUG: Created by: '$createdBy'");

        if (empty($id)) {
            $id = 'PAT-' . time() . '-' . rand(1000, 9999);
            error_log("DEBUG: Generated new patient ID: '$id'");
        }
        ...
        $result = $stmt->execute([$id, $name, $age, $gender, $phone, $email, $address, $emergencyContact, $emergencyPhone, $allergies, $medicalConditions, $currentMedications, $branchId, $createdBy]);
        error_log('DEBUG: INSERT executed, result: ' . ($result ? 'true' : 'false'));
        ...
        $patient = [
            ...
            'medicalConditions' => json_decode($medicalConditions, true) ?? [],
            'currentMedications' => json_decode($currentMedications, true) ?? [],
            'branchId' => $branchId,
