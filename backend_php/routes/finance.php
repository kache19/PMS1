...
function createInvoice() {
    ...
    try {
        ...
        error_log("createInvoice params: id=$id, branchId=$branchId, customerName=$customerName, totalAmount=$totalAmount");

        // Validate required fields
        if (empty($id) || empty($branchId) || empty($customerName)) {
            error_log('Missing required fields');
            http_response_code(400);
            echo json_encode(['error' => 'Missing required fields: id, branchId, customerName']);
            return;
        }
        ...
        $result = $stmt->execute([$id, $branchId, $customerName, $totalAmount, 'UNPAID', $dueDate, $description, $source, $itemsJson]);
        ...
        $response = json_encode(['message' => 'Invoice created successfully', 'invoiceId' => $id]);
        if ($response === false) {
