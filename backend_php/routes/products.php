...
function getProducts() {
    ...
    try {
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'ACCOUNTANT', 'INVENTORY_CONTROLLER', 'PHARMACIST', 'CASHIER', 'DISPENSER', 'STOREKEEPER', 'AUDITOR']);
        ...
        $result = array_map(function($p) {
            return [
                ...
                'unit' => $p['unit'],
                'minStockLevel' => (int)$p['min_stock_level'],
                'requiresPrescription' => (bool)$p['requires_prescription'],
                'totalStock' => 0, // Will be calculated on frontend
                'batches' => [] // Batches are in separate table
            ];
        }, $products);
...
function getProduct($id) {
    ...
    try {
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'ACCOUNTANT', 'INVENTORY_CONTROLLER', 'PHARMACIST', 'DISPENSER', 'STOREKEEPER']);
        ...
        $result = [
            ...
            'requiresPrescription' => (bool)$product['requires_prescription'],
            'totalStock' => 0, // Will be calculated on frontend
            'batches' => [] // Batches are in separate table
        ];
...
function updateProduct($id) {
    ...
    try {
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'INVENTORY_CONTROLLER']);
        $input = json_decode(file_get_contents('php://input'), true);
        error_log('Update product input: ' . json_encode($input));
        ...
        foreach ($fields as $jsonKey => $dbKey) {
            if (isset($input[$jsonKey])) {
                $updates[] = "$dbKey = ?";
                if ($jsonKey === 'costPrice' || $jsonKey === 'price') {
                    $values[] = (float)$input[$jsonKey];
                } elseif ($jsonKey === 'minStockLevel') {
                    $values[] = (int)$input[$jsonKey];
                } elseif ($jsonKey === 'requiresPrescription') {
                    $values[] = (bool)$input[$jsonKey] ? 1 : 0;
                } else {
