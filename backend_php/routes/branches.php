...
function updateBranch($id) {
    ...
    try {
        ...
        if (isset($input['is_head_office'])) {
            if ($input['is_head_office']) {
                // First, unset any existing head office
                $pdo->prepare('UPDATE branches SET is_head_office = 0 WHERE is_head_office = 1')->execute();

                // Set this branch as head office
                $updates[] = 'is_head_office = ?';
                $values[] = 1;
            } else {
                // Unset head office for this branch
                $updates[] = 'is_head_office = ?';
                $values[] = 0;
            }
        }
...
function getBranchDetails($id) {
    ...
    try {
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'ACCOUNTANT', 'DISPENSER', 'PHARMACIST', 'INVENTORY_CONTROLLER', 'STOREKEEPER']);
...
function getBranchStaff($id) {
    ...
    try {
        authorizeRoles(['SUPER_ADMIN', 'BRANCH_MANAGER', 'ACCOUNTANT', 'DISPENSER']);
